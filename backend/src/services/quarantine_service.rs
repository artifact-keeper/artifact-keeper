...
/// Resolve the effective quarantine config for a repository. Per-repo
/// overrides in the `repository_config` table take precedence over the
/// global defaults from the application config.
pub async fn resolve_repo_quarantine_config(
    db: &sqlx::PgPool,
    repo_id: uuid::Uuid,
    global: &QuarantineConfig,
) -> QuarantineConfig {
    let rows: Vec<(String, String)> = sqlx::query_as(
        "SELECT key, value FROM repository_config WHERE repository_id = $1 AND key IN ('quarantine_enabled', 'quarantine_duration_minutes')",
    )
    .bind(repo_id)
    .fetch_all(db)
    .await
