...
use crate::api::handlers::cache_headers::{
    check_conditional_request, compute_etag, DEFAULT_CACHE_CONTROL,
};
...
fn cached_packument_response(entry: &CachedPackument, request_headers: &HeaderMap) -> Response {
    if let Some(not_modified) = check_conditional_request(request_headers, &entry.etag) {
        return not_modified;
    }
...
///
/// `Vary` is required here for the same reason the cached path sets it: `Accept`
/// selects between the full and abbreviated packument, so a shared cache told it
/// may store this response must also be told the response varies by `Accept`, or
/// it could serve an abbreviated packument to a client that asked for the full
/// one. Non-`200` and non-JSON responses are passed through untouched.
...
async fn packument_response_with_cache_headers(
    ...
    if let Some(not_modified) = check_conditional_request(request_headers, &etag) {
...
mod tests {
    ...
    fn test_cached_packument_response_headers() {
        ...
        assert_eq!(response.headers()[CACHE_CONTROL], DEFAULT_CACHE_CONTROL);
    ...
    fn test_cached_packument_response_survives_corrupt_header_values() {
        ...
        // An unsettable ETag degrades to an uncacheable response rather than
        // emitting Cache-Control with no tag to revalidate against.
...
mod db_cov_tests {
    ...
    async fn test_remote_packument_carries_etag_and_answers_304() {
        use crate::api::handlers::cache_headers::DEFAULT_CACHE_CONTROL;
