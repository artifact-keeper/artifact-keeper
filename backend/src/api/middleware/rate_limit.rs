...
/// Set of usernames and service-account flags that bypass rate limiting.
///
/// Backported from #697. Exempts well-known shared accounts (e.g. the
/// built-in `admin` user in CI/test environments) from the in-process
/// limiter so bcrypt verification under burst load does not produce
/// 401/429 flakes.
#[derive(Debug, Clone)]
...
impl RateLimitExemptions {
    ...
    /// Whether the supplied auth context should bypass the rate limiter.
    ...
    /// True when no exemption rules are configured.
    pub fn is_empty(&self) -> bool {
        self.usernames.is_empty() && !self.exempt_service_accounts
    }
}

/// Combined limiter + exemption rules used as middleware state.
#[derive(Debug, Clone)]
...
pub async fn rate_limit_middleware(
    ...
    // Pull auth out of extensions for both the required-auth and optional-auth
    // middleware shapes. We need this to (1) decide whether the principal is
    // exempt and (2) build the per-user rate-limit key.
    let auth = request
        .extensions()
        .get::<AuthExtension>()
        .cloned()
    ...
    // Bypass the limiter entirely for exempt principals (e.g. shared admin
    // account in CI). Annotate the response so callers can see the bypass.
...
mod tests {
    ...
    #[test]
    fn test_exemptions_admin_username_bypass() {
        // The CI/test scenario that motivated the backport: shared `admin`
        // login under burst auth. Confirm the username matcher picks it up.
        let ex = RateLimitExemptions::new(vec!["admin".into()], false);
        assert!(ex.is_exempt(&make_auth("admin", false)));
        assert!(!ex.is_exempt(&make_auth("alice", false)));
    }
}
