...
- **Security scanner silent-success on type-mismatched scans** (#994) -- the scan orchestrator (`ScannerService::scan_artifact_with_options`) ran every registered scanner against every artifact and recorded a `scan_results` row regardless of whether a given scanner could actually inspect the artifact format. Scanners that did not apply (e.g. `ImageScanner` against an npm tarball, `IncusScanner` against a `.crate`, `OpenScapScanner` against a `.whl`) returned `Ok(vec![])` from `scan()`, which the orchestrator treated as a successful zero-finding scan and persisted with `status='completed'`, `findings_count=0`, `scanner_version=NULL`, and `error_message=NULL`. The rows are indistinguishable from a real clean scan, so receivers using a `scan_results` row as a security gate (`/api/v1/security/artifacts/{id}/scans`) waved through artifacts that had never actually been scanned. The release-gate `tests/security/test-scan-completes.sh` flagged this on a `lodash 4.17.4` fixture that has multiple known CVEs but produced `findings_count=0` with `completed_at - started_at = 2.8ms`, far too short for a real Trivy scan. Fix introduces `Scanner::is_applicable(&Artifact, Option<&ArtifactMetadata>) -> bool` on the `Scanner` trait (defaults to `true` so the always-on dependency scanner is unaffected) and gates `scan_results` row creation on it. Non-applicable scanners now leave no DB trace, so a `completed` row is a real signal that the scanner ran. Scan-time short-circuits inside each scanner are kept as defense in depth for direct callers and tests.
