...
- **Legacy flat-key Maven reads restored on cloud backends via a catalog ownership check** (#2574): the v1.5.7 fix for #2504 gated every unanchored `maven/{path}` storage read (row-less GAV companion files, verbatim `maven-metadata.xml`, stored checksum sidecars, the hosted direct-byte fallback, the `maven_proxy` storage fallback) to repo-isolated filesystem backends, which broke serving of pre-existing data on S3/GCS/Azure — companion files of GAVs uploaded by older releases (`.pom`, `.module`, `-sources.jar`, …) 404'd after the upgrade, and deliberately-uploaded metadata silently degraded to synthesized documents. These reads now apply the **same catalog ownership rule as the #2504 write guard** (backed by `idx_artifacts_storage_key`): a flat key referenced by a *different* repository — live or soft-deleted — stays unreadable (the cross-tenant hole remains closed), while keys that are unowned or owned by the requesting repository serve as they did before v1.5.7. Database errors fail closed. No configuration or migration required.
