...
- **New admin endpoint, `POST /api/v1/admin/age-gate/reviews/{id}/reopen`, to return an approved or rejected age-gate review to `pending`** (#2939). Approve and reject were both terminal, so an admin who approved the wrong package could only recover by editing `age_gate_reviews` directly in Postgres. Reopen requires a non-blank `reason` (400 otherwise, and 400 on a review that is already pending) and records the reversal under a new `AGE_GATE_REOPENED` audit action carrying the decision it undid. Returning the review to `pending` re-applies the gate: a version that is still younger than the threshold is withheld from downloads and from the npm packument and PyPI simple index again, while one that has since aged past the threshold auto-approves on the next request.
