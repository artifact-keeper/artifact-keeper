...
### Fixed
- **Remote cargo `.crate` downloads stream instead of buffering at the 8 MiB metadata cap**, so crates above that size no longer fail with a 502 (#895 / #2192 class). Upstream `Content-Encoding` is now forwarded on both the Remote and Virtual download arms, so a content-coded `.crate` from an intermediary is declared rather than silently written to disk compressed and failing cargo's checksum. Two operator-visible changes come with the switch, both matching the ~25 formats already converted: `Content-Length` is omitted when upstream does not advertise one (the response is then chunked), and a Remote cargo repository with the **Package Age Policy enabled now returns 409 for any crate not already cached**, rather than fetching it and applying the hold. Streaming has no buffered `Last-Modified` to compute a release-date hold from, so the fetch is refused instead (#1770 / #1771); because nothing is cached, the hold does not elapse on its own. The 409 body reads "Artifact is quarantined and pending security review", which refers to this policy, not to a scanner verdict. Note `QUARANTINE_ENABLED` acts as a global default, so this can affect every Remote cargo repository at once.
