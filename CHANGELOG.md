...
- **System-wide TOTP (2FA) enrollment policy** (#2805): TOTP was fully implemented per user but strictly opt-in. A new `TOTP_POLICY` env var (`disabled` | `required_for_admins` | `required_for_all`, default `disabled`) lets operators require enrollment. When a user subject to the policy has not enrolled, login still succeeds but the session is restricted, exactly like the forced password-change flow: every request other than the TOTP enrollment endpoints (`POST /auth/totp/setup`, `POST /auth/totp/enable`), the self lookup (`GET /auth/me`), self password change, and logout is refused with `428 Precondition Required` until enrollment completes, so no one is locked out. The login response carries a `must_enroll_totp` flag so clients can route into enrollment. SSO-authenticated users (LDAP / SAML / OIDC / CI) are exempt because their identity provider owns MFA, and non-interactive principals (API tokens, service accounts) are exempt because they are not interactive logins. The default (`disabled`) is byte-for-byte unchanged behaviour.
