...
- **Scanner dedup TTL no longer silently masks re-scans when a prior completed scan had zero findings** (closes #1469). `find_reusable_scan` and `find_existing_scan_for_artifact` previously applied a uniform 30-day TTL to every `status='completed'` row that matched `(checksum_sha256, scan_type)`. The uniform window has a hole: a scan that finished successfully but produced zero findings is indistinguishable, at the `scan_results` table level, from (1) a genuinely clean artifact and (2) a silent extraction failure where the staging tree was empty so the scanner walked nothing (the incus backup-format bug in #1427 / #1428 is the canonical instance). Case (2) gets cached for 30 days, so the operator-visible "rescan after fixing the extraction bug" silently returns the cached false-clean result. Two changes resolve this: (a) both dedup queries now take a second `zero_findings_ttl_days` argument and apply it via `CASE WHEN findings_count = 0 THEN ... ELSE ... END`, with the production callers passing the new `ZERO_FINDINGS_DEDUP_TTL_DAYS = 1` constant; zero-finding rows older than one day are no longer returned as reusable, while non-zero-finding rows still dedup for the standard 30 days, so legitimate vulnerability data continues to be reused; (b) `TriggerScanRequest` gains an optional `bypass_dedup: bool` (default `false`) that threads down through `prepare_artifact_scan`, `scan_artifact_with_options`, `scan_artifact_with_prepared`, `scan_repository_with_options`, and `scan_artifact_inner` to short-circuit both the same-artifact and cross-artifact dedup paths entirely, giving operators an explicit "rescan now, ignore the cache" knob for the impatient case. The inventory-backfill admin path (`POST /api/v1/admin/scan-inventory-backfill`) now passes `bypass_dedup = true` because its whole purpose is to populate SBOM rows that the cached false-clean had left empty. Four DB-backed regression tests in `tests/scan_dedup_zero_findings_ttl_tests.rs` cover the stale-zero-finding skip, the within-window keep, the unchanged non-zero-finding path, and the operationally-important "newer stale zero must not displace an older real-finding row" ordering case. Two `TriggerScanRequest` serde tests pin the new field's default-omitted and explicit-true/false shapes.
