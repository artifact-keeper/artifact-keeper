...
**First-boot setup**: A random admin password is written to `/data/storage/admin.password` inside the container. The API is locked until you change it. Run `docker exec artifact-keeper-backend cat /data/storage/admin.password` to retrieve it. Set `ADMIN_PASSWORD` in your `.env` to skip the setup lock.
